...
from src.config import PUTER_BASE_URL, PUTER_API_KEY, CUSTOM_LLM_BASE_URL, CUSTOM_LLM_API_KEY, LLM_PROVIDER, DEFAULT_MODEL
...
logger = logging.getLogger("DinacharyaPlannerAgent")
...
class DinacharyaPlannerAgent:
    ...
    Implements the Plan-and-Execute pattern using LangGraph and our Custom LLM Provider.
    """

    def __init__(self):
        # Initialize OpenAI client pointed to our Custom LLM / configured provider
  ...
  "evening_block": [...]
    ...
    def executor_node(self, state: AgentState) -> AgentState:
        ...
        for block_name in ["morning_block", "midday_block", "evening_block"]:
            for practice in state.get(block_name, []):
